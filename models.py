...
class StochasticDurationPredictor(nn.Module):
    def __init__(
        ...
        super().__init__()
        filter_channels = in_channels  # it needs to be removed from future versio
...
class SynthesizerTrn(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if n_speakers > 1:
            self.emb_g = nn.Embedding(n_speakers, gin_channels)  # Speaker embedding
            self.gst = GST(hparams)  # GST for style embedding
            self.proj = nn.Conv1d(2 * gin_channels, gin_channels, 1)  # Combine embeddings

    def forward(self, x, x_lengths, y, y_lengths, sid=None):
        if self.n_speakers > 0:
            g_speaker = self.emb_g(sid).unsqueeze(-1)  # [b, gin_channels, 1]
            g_style = self.gst(y, y_lengths).unsqueeze(-1)  # Use target mel as reference [b, gin_channels, 1]
            g_combined = torch.cat([g_speaker, g_style], dim=1)  # [b, 2*gin_channels, 1]
            g = self.proj(g_combined)  # [b, gin_channels, 1]
        else:
            g = None
